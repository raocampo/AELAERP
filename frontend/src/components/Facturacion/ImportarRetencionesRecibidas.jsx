// ====================================
// IMPORTAR RETENCIONES RECIBIDAS DESDE EXCEL
// frontend/src/components/Facturacion/ImportarRetencionesRecibidas.jsx
// ====================================
...
export default function ImportarRetencionesRecibidas({ onImportado }) {
  ...
  const descargarPlantilla = async () => {
    try {
      const resp = await api.get('/retenciones-recibidas/importar/plantilla', { responseType: 'blob' });
      const url  = window.URL.createObjectURL(new Blob([resp.data]));
      const a    = document.createElement('a');
      a.href     = url;
      a.setAttribute('download', 'plantilla-retenciones-recibidas.xlsx');
  ...
  const cargarPreview = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/retenciones-recibidas/importar/preview', fd);
      setPreview(data);
      setPaso(2);
    } catch (err) {
  ...
  const ejecutarImportacion = async () => {
    if (!archivo) return;
    if (!window.confirm(`¿Confirmar la importación de ${preview?.validas || 0} retenciones válidas? Esta acción no se puede deshacer.`)) return;
    ...
    try {
      const fd = new FormData();
      fd.append('archivo', archivo);
      const { data } = await api.post('/retenciones-recibidas/importar/ejecutar', fd);
      setResultado(data);
      setPaso(3);
      if (data.importadas > 0) onImportado?.();
    } catch (err) {
      setError(err.response?.data?.mensaje || 'Error durante la importación');
    } finally {
      setCargando(false);
    }
  };
          ...
          <div className="ifh-info-grid">
            <div className="ifh-info-item">
              ...
              <div>
                ...
                <p>Tiene las mismas columnas que exporta el SRI en "Comprobantes recibidos" — si ya tienes ese archivo, úsalo directo.</p>
              </div>
            </div>
            <div className="ifh-info-item">
              <span className="ifh-info-num">2</span>
              <div>
                <strong>Una fila por comprobante</strong>
                <p>Necesitas la clave de acceso (49 dígitos), el agente de retención, la fecha y al menos un valor retenido (Renta o IVA).</p>
              </div>
            </div>
            <div className="ifh-info-item">
            ...
            <div className="ifh-info-item">
              ...
              <div>
                ...
                <p>Solo se importan las filas válidas. Cada una genera su asiento contable en el Libro Diario.</p>
              </div>
            </div>
          </div>

          <div className="ifh-nota">
            <strong>Importante:</strong> si la clave de acceso ya existe (por ejemplo, ya se importó desde
            el Buzón SRI), esa fila se omite automáticamente — no se duplica.
          </div>
        ...
        <div className="ifh-card">
              ...
              <tbody>
                {preview.filas.map((f) => (
                  <tr key={f.fila} className={f.valida ? '' : 'ifh-fila-error'}>
                    ...
                    {f.valida ? (
                      <>
                        <td>{f.datos.fechaEmision ? new Date(f.datos.fechaEmision).toLocaleDateString('es-EC') : '-'}</td>
                        <td className="ifh-td-cliente" title={f.datos.razonSocialAgente}>{f.datos.razonSocialAgente}</td>
                        <td>{f.datos.numDocSustento || '-'}</td>
                        <td>{f.datos.totalRetencionRenta > 0 ? `$${f.datos.totalRetencionRenta.toFixed(2)}` : '-'}</td>
                        <td>{f.datos.totalRetencionIva > 0 ? `$${f.datos.totalRetencionIva.toFixed(2)}` : '-'}</td>
                      </>
                    ) : (
                      <td colSpan={5} className="ifh-errores-celda">
                        {f.errores.join(' · ')}
                      </td>
                    )}
                  </tr>
          ...
          {preview.validas > 0 && (
            <div className="ifh-resumen-totales">
              ...
              ${preview.filas.filter(f => f.valida).reduce((s, f) => s + (f.datos.totalRetencionRenta || 0) + (f.datos.totalRetencionIva || 0), 0).toFixed(2)}{' '}
              en {preview.validas} retenciones
            </div>
          )}
          ...
          <div className="ifh-acciones">
            ...
            {preview.validas > 0 && (
              <button className="btn-primary" onClick={ejecutarImportacion} disabled={cargando}>
                {cargando ? 'Importando...' : `Importar ${preview.validas} retenciones →`}
              </button>
            )}
          </div>
        </div>
      ...
      {paso === 3 && resultado && (
        <div className="ifh-card">
          <div className={`ifh-resultado-header ${resultado.importadas > 0 ? 'ifh-resultado-ok' : 'ifh-resultado-err'}`}>
            ...
            <div>
              <h2>{resultado.importadas > 0 ? 'Importación completada' : 'Sin retenciones importadas'}</h2>
              <p>
                {resultado.importadas > 0 && <><strong>{resultado.importadas}</strong> retenciones importadas exitosamente. </>}
          ...
          {resultado.detalle?.importadas?.length > 0 && (
            <div className="ifh-resultado-seccion">
              <h3>Retenciones importadas</h3>
              <table className="ifh-tabla ifh-tabla-compact">
                <thead>
                  <tr><th>Fila</th><th>Agente</th><th>Total</th><th>Libro Diario</th></tr>
                </thead>
                <tbody>
                  {resultado.detalle.importadas.map(r => (
                    <tr key={r.id}>
                      ...
                      <td>
                        {r.asientoOk
                          ? <span className="ifh-badge ifh-badge-autorizado" title="Se generó el asiento en el Libro Diario">✓ Enlazada</span>
                          : <span className="ifh-badge ifh-badge-historico" title="No se pudo generar el asiento contable">⚠ Sin asiento</span>
                        }
                      </td>
                    </tr>
                  ))}
