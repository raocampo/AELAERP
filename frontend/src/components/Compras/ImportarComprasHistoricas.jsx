// ====================================
// IMPORTAR FACTURAS DE COMPRA HISTÓRICAS
// frontend/src/components/Compras/ImportarComprasHistoricas.jsx
// ====================================

import { useRef, useState } from 'react';
import api from '../../services/api';
import '../Facturacion/ImportarFacturasHistoricas.css';
...
export default function ImportarComprasHistoricas() {
  ...
  const descargarPlantilla = async () => {
    try {
      const resp = await api.get('/compras/importar/plantilla', { responseType: 'blob' });
      const url  = window.URL.createObjectURL(new Blob([resp.data]));
      const a    = document.createElement('a');
      a.href     = url;
      a.setAttribute('download', 'plantilla-compras-historicas.xlsx');
  ...
  const cargarPreview = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/compras/importar/preview', fd);
      setPreview(data);
      setPaso(2);
    } catch (err) {
  ...
  const ejecutarImportacion = async () => {
    if (!archivo) return;
    if (!window.confirm(`¿Confirmar la importación de ${preview?.validas || 0} facturas de compra válidas? Esta acción no se puede deshacer.`)) return;
    ...
    try {
      const fd = new FormData();
      fd.append('archivo', archivo);
      const { data } = await api.post('/compras/importar/ejecutar', fd);
      setResultado(data);
      setPaso(3);
    } catch (err) {
  ...
  return (
    <div className="ifh-container">
      <div className="ifh-header">
        <h1 className="ifh-titulo">Importar Facturas de Compra Históricas</h1>
        <p className="ifh-subtitulo">
          Carga compras/gastos a proveedores de años anteriores desde Excel para llevar la contabilidad completa en AELA.
        </p>
      </div>
      ...
      {paso === 0 && (
        <div className="ifh-card">
          ...
          <div className="ifh-info-grid">
            ...
            <div className="ifh-info-item">
              ...
              <div>
                ...
                <p>Una fila por factura de compra. Necesitas proveedor, número de factura, fecha y montos.</p>
              </div>
            </div>
            <div className="ifh-info-item">
          ...
          <div className="ifh-nota">
            <strong>Importante:</strong> esta importación NO afecta el inventario ni crea productos —
            es solo para efectos contables (gasto/compra ya consumido). El número de factura del
            proveedor es obligatorio: es el documento original, AELA no lo asigna.
          </div>
        ...
        <div className="ifh-card">
          ...
          {preview.validas > 0 && (
            <div className="ifh-resumen-totales">
              ...
              en {preview.validas} facturas de compra
            </div>
          )}
          ...
          <div className="ifh-acciones">
            ...
            {preview.validas > 0 && (
              <button className="btn-primary" onClick={ejecutarImportacion} disabled={cargando}>
                {cargando ? 'Importando...' : `Importar ${preview.validas} facturas de compra →`}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="ifh-card">
          <div className={`ifh-resultado-header ${resultado.importadas > 0 ? 'ifh-resultado-ok' : 'ifh-resultado-err'}`}>
            ...
            <div>
              ...
              <p>
                {resultado.importadas > 0 && <><strong>{resultado.importadas}</strong> facturas de compra importadas exitosamente. </>}
          ...
          {resultado.detalle?.importadas?.length > 0 && (
            <div className="ifh-resultado-seccion">
              <h3>Facturas de compra importadas</h3>
              <table className="ifh-tabla ifh-tabla-compact">
                <thead>
                  <tr><th>Fila</th><th>Proveedor</th><th>N° Factura</th><th>Total</th><th>Libro Diario</th></tr>
                </thead>
                <tbody>
                  {resultado.detalle.importadas.map(r => (
                    <tr key={r.id}>
                      ...
                      <td>
                        {r.asientoOk
                          ? <span className="ifh-badge ifh-badge-autorizado" title="Se generó el asiento de compra en el Libro Diario">✓ Enlazada</span>
              ...
              <p className="ifh-nota-diario">
                💡 Cada factura de compra importada genera automáticamente su asiento en el
                <strong> Libro Diario</strong> (Contabilidad → Libro Diario). No se ajusta
                inventario ni se crean productos — es solo para efectos contables.
              </p>
            </div>
          )}
          ...
          <div className="ifh-acciones">
            ...
            <a href="/compras" className="btn-primary">Ver facturas de compra →</a>
          </div>
        </div>
      )}
    </div>
