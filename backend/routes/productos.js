  ...
  try {
    ...
    const item = await prisma.productos_servicios.create({
      data: {
        ...
        unidadesPorPaquete: Math.max(1, parseInt(unidadesPorPaquete ?? 1, 10) || 1),
        nombrePaquete: nombrePaquete?.trim() || null,
        precioPaquete: precioPaquete !== undefined && precioPaquete !== null && precioPaquete !== ''
          ? parseFloat(precioPaquete) : null,
  ...
  try {
    ...
    const item = await prisma.productos_servicios.update({
      ...
      data: {
        ...
        ...(unidadesPorPaquete !== undefined ? { unidadesPorPaquete: Math.max(1, parseInt(unidadesPorPaquete, 10) || 1) } : {}),
        ...(nombrePaquete !== undefined ? { nombrePaquete: nombrePaquete?.trim() || null } : {}),
        ...(precioPaquete !== undefined ? { precioPaquete: precioPaquete === null || precioPaquete === '' ? null : parseFloat(precioPaquete) } : {}),
