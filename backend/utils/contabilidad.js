...
async function crearAsientoFacturaCompraRegistrada({ compraId, usuarioId, fecha = new Date(), db = prisma }) {
  ...
  // Cuando el usuario asigna explícitamente una cuenta de gasto (cuentaGastoId), TODO
  // el importe va a esa cuenta — ignoramos si los ítems son inventariables o no.
  // Sin cuenta explícita, separamos inventariable vs gasto por línea de detalle.
  const detallesCompra = Array.isArray(compra.detalles) ? compra.detalles : [];
  let subtotalInventario = compra.cuentaGastoId ? 0 : round2(detallesCompra
